...
function Shell({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-2 gap-3">{children}</div>
    </div>
  );
}
...
function makeRow(): (string | null)[] { return Array.from({ length: COLS }, () => rndColor()); }
function initGrid(rows: number): (string | null)[][] { return Array.from({ length: rows }, () => makeRow()); }
...
function floodFill(grid: (string | null)[][], r: number, c: number): [number, number][] {
  ...
  const vis = new Set<string>(); const q: [number, number][] = [[r, c]]; const out: [number, number][] = [];
  while (q.length) {
    const [cr, cc] = q.pop()!; const k = `${cr},${cc}`;
    if (vis.has(k) || cr < 0 || cr >= grid.length || cc < 0 || cc >= COLS || grid[cr]?.[cc] !== color) continue;
    vis.add(k); out.push([cr, cc]); cellNbrs(cr, cc).forEach(n => q.push(n));
  }
  return out;
}

function findDetached(grid: (string | null)[][]): [number, number][] {
  const conn = new Set<string>(); const q: [number, number][] = [];
  for (let c = 0; c < COLS; c++) { if (grid[0]?.[c]) { conn.add(`0,${c}`); q.push([0, c]); } }
  while (q.length) {
    const [r, c] = q.pop()!;
...
type GS = {
  grid: (string | null)[][]; color: string; nextColor: string; angle: number;
  proj: { x: number; y: number; vx: number; vy: number; color: string } | null;
  score: number; state: "playing" | "lose"; lastAdd: number; interval: number;
};
function mkGS(d: Difficulty): GS {
  const cfg = DIFF[d];
  return { grid: initGrid(cfg.rows), color: rndColor(), nextColor: rndColor(), angle: -Math.PI / 2, proj: null, score: 0, state: "playing", lastAdd: Date.now(), interval: cfg.interval };
}
...
export default function BubbleShooter() {
  ...
  // Convert canvas-relative coordinates accounting for CSS scaling
  const toCanvas = useCallback((clientX: number, clientY: number) => {
    const el = cv.current; if (!el) return { x: clientX, y: clientY };
  ...
  const draw = useCallback(() => {
    ...
    ctx.fillStyle = frac > 0.35 ? "#22d3ee" : "#f43f5e"; ctx.fillRect(8, 26, frac * 80, 5);
    ctx.fillStyle = "#475569"; ctx.font = "9px sans-serif"; ctx.fillText("next row ▼", 8, 40);
  }, []);
  ...
  const applyHit = useCallback((px: number, py: number, color: string, hitR: number, hitC: number) => {
    ...
    const cands = cellNbrs(hitR, hitC).filter(([r, c]) => {
      if (r < 0) return false;
      if (r >= s.grid.length) return true;
      return !s.grid[r]?.[c];
    });
    ...
    setScore(s.score);
    if (s.grid.length > 0 && by(s.grid.length - 1) + R >= DANGER_Y) { s.state = "lose"; setGState("lose"); }
  }, []);
  ...
  const loop = useCallback(() => {
    ...
    if (p) {
      ...
      if (p.y <= R + TOP_PAD) {
        placeAtTop(p.x, p.color); s.proj = null; s.color = s.nextColor; s.nextColor = rndColor();
      ...
      if (hitR >= 0) { applyHit(p.x, p.y, p.color, hitR, hitC); s.proj = null; s.color = s.nextColor; s.nextColor = rndColor(); }
    }
    draw(); raf.current = requestAnimationFrame(loop);
  }, [draw, applyHit, placeAtTop]);

  const shoot = useCallback(() => {
    const s = g.current; if (s.proj || s.state !== "playing") return;
    s.proj = { x: SHOOTER_X, y: SHOOTER_Y, vx: Math.cos(s.angle) * 12, vy: Math.sin(s.angle) * 12, color: s.color };
  }, []);

  const aimAt = useCallback((cx: number, cy: number) => {
    const { x, y } = toCanvas(cx, cy);
    const ang = Math.atan2(y - SHOOTER_Y, x - SHOOTER_X);
    g.current.angle = Math.max(-Math.PI + 0.15, Math.min(-0.15, ang));
  }, [toCanvas]);

  const startGame = useCallback((d: Difficulty) => {
    g.current = mkGS(d); setDiff(d); setScore(0); setGState("playing");
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent) => {
      e.preventDefault();
      const t = e.touches[0]; aimAt(t.clientX, t.clientY);
    };
        ...
        <div className="flex flex-col gap-3 w-full">
          {(["easy", "medium", "hard"] as Difficulty[]).map(d => (
            <button key={d}
              ...
              className={`py-4 px-6 rounded-2xl border font-black transition-colors text-left touch-manipulation ${
              ...
              <span className="text-lg">{DIFF[d].label}</span><br />
  ...
  return (
    <Shell title="Bubble Shooter">
        ...
        {gstate === "lose" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 rounded-xl gap-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => startGame(diff)} className="px-8 py-3 bg-pink-500 text-white font-bold rounded-xl touch-manipulation">Play Again</button>
      ...
      <p className="text-xs text-muted-foreground text-center">Touch &amp; drag to aim · release to shoot · ← → keys or mouse also work</p>
    </Shell>
  );
}
