...
// ─── TRACKS ────────────────────────────────────────────────────────────────
// Each track is a closed loop of centerline waypoints.
// Difficulty increases with more turns, narrower road, sharper corners.

const TRACKS: TrackDef[] = [
  {
    // Inspired by Autodromo Nazionale Monza — very fast, 2 chicanes, mostly right-handers
    ...
    wps: [
      ...
      { x: 428, y:  76 }, { x: 476, y: 124 }, // LEFT/RIGHT chicane
      { x: 562, y: 100 }, { x: 618, y: 148 }, // Curva Grande entry (R)
      { x: 638, y: 235 }, { x: 638, y: 388 }, // right side straight
      { x: 596, y: 460 }, { x: 478, y: 496 }, // Parabolica (R)
      { x: 330, y: 496 }, { x: 165, y: 496 }, // bottom straight
      { x:  82, y: 450 }, { x:  62, y: 325 }, // Lesmo corners (L then R)
      { x:  68, y: 200 }, { x: 116, y: 128 }, // back to pit straight
    ],
  },
  {
    // Inspired by Silverstone — flowing medium-speed corners, famous Maggotts-Becketts-Chapel L-R-L-R complex
    ...
    wps: [
      ...
      { x: 465, y: 158 }, // Copse (R)
      { x: 530, y: 132 }, { x: 586, y: 158 }, // Maggotts L → Becketts R
      { x: 632, y: 130 }, { x: 655, y: 172 }, // Becketts L → Chapel R
      { x: 655, y: 278 }, // Hangar straight
      { x: 632, y: 375 }, // Stowe (R)
      { x: 578, y: 440 }, { x: 468, y: 478 }, // Vale–Club (R)
      { x: 328, y: 492 }, { x: 196, y: 470 }, // Abbey (L)
      { x: 132, y: 414 }, { x: 108, y: 345 }, // Farm curve (L)
    ],
  },
  {
    // Inspired by Spa-Francorchamps — Eau Rouge L-R sequence, Pouhon long lefts, Bus Stop chicane
    ...
    wps: [
      { x: 188, y: 115 }, { x: 375, y: 115 }, { x: 520, y: 115 }, // Kemmel straight →
      { x: 575, y: 158 }, { x: 600, y: 210 }, // La Source hairpin (R)
      { x: 594, y: 276 }, { x: 560, y: 310 }, // Eau Rouge (L)
      { x: 582, y: 364 }, { x: 592, y: 435 }, // Raidillon back up (R)
      { x: 566, y: 494 }, { x: 496, y: 512 }, // Les Combes (R)
      { x: 392, y: 500 }, { x: 290, y: 480 }, // Pouhon entry (L)
      { x: 210, y: 448 }, { x: 146, y: 384 }, // Pouhon long left (L)
      { x:  94, y: 305 }, { x:  76, y: 226 }, // Bruxelles (L)
      { x:  96, y: 160 }, { x: 144, y: 128 }, // Bus Stop chicane R–L
    ],
  },
  {
    // Inspired by Suzuka — S-curves (4-direction alternation), 130R, Spoon, hairpin
    id: "suzuka", name: "Suzuka", flag: "🇯🇵", stars: 4,
    desc: "S-curves R-L-R-L · 130R · Spoon · Hairpin",
    trackW: 60, wpR: 38, aiDiff: "hard",
    wps: [
      { x: 218, y: 112 }, { x: 395, y: 112 }, // main straight →
      { x: 478, y: 142 }, { x: 518, y: 196 }, // Turns 1–2 (R)
      { x: 522, y: 262 },                       // straight into S
      { x: 550, y: 292 }, { x: 508, y: 322 },  // S-curve R → L
      { x: 550, y: 352 }, { x: 508, y: 382 },  // S-curve R → L (second pair)
      { x: 478, y: 430 }, { x: 414, y: 456 },  // Hairpin (R)
      { x: 346, y: 440 }, { x: 276, y: 402 },  // out of hairpin
      { x: 216, y: 350 }, { x: 170, y: 276 },  // 130R / Spoon (L)
      { x: 120, y: 210 }, { x: 126, y: 148 },  // Casio Triangle (R then L)
      { x: 174, y: 118 },
    ],
  },
  {
    // Inspired by Circuit de Monaco — ultra-tight street circuit, 22 corners, all types
    id: "monaco", name: "Monaco", flag: "🇲🇨", stars: 5,
    desc: "Street circuit · 22 turns · very narrow & technical",
    trackW: 54, wpR: 30, aiDiff: "hard",
    wps: [
      { x: 172, y: 258 }, { x: 298, y: 258 },  // pit straight →
      { x: 360, y: 232 }, { x: 400, y: 202 },  // Sainte Devote (R)
      { x: 446, y: 180 }, { x: 494, y: 170 },  // uphill (R)
      { x: 536, y: 192 }, { x: 562, y: 234 },  // Casino Square (R)
      { x: 556, y: 282 }, { x: 524, y: 318 },  // Mirabeau / Station (L)
      { x: 568, y: 364 }, { x: 606, y: 394 },  // Portier (R)
      { x: 634, y: 434 },                        // tunnel entry
      { x: 610, y: 476 }, { x: 562, y: 472 },  // Nouvelle Chicane (L)
      { x: 528, y: 494 }, { x: 458, y: 504 },  // Tabac (R then L)
      { x: 390, y: 494 }, { x: 324, y: 504 },  // Swimming Pool L–R
      { x: 260, y: 492 }, { x: 212, y: 462 },  // Swimming Pool exit (R)
      { x: 163, y: 437 }, { x: 136, y: 396 },  // Rascasse (R)
      { x: 148, y: 336 }, { x: 154, y: 294 },  // Anthony Noghes (R)
    ],
  },
];
...
function drawTrack(ctx: CanvasRenderingContext2D, track: TrackDef) {
  ...
  // Draw thick closed stroke for road
  ...
  tracePath(); ctx.strokeStyle = "rgba(255,255,255,0.03)"; ctx.lineWidth = trackW * 2 - 8; ctx.stroke();
...
type Car = {
  ...
  color: string; lap: number; wpIdx: number;
  bestLap: number; lapStart: number; label: string;
};
  ...
  const loop = useCallback(() => {
    ...
    const updateCar = (c: Car, left: boolean, right: boolean, up: boolean, down: boolean, maxSpd = 5.5): boolean => {
      ...
      if (Math.sqrt((c.x - wp.x) ** 2 + (c.y - wp.y) ** 2) < wpR) {
        ...
        if (c.wpIdx === 0) {
          c.wpIdx = 1; c.lap++;
          const lt = Date.now() - c.lapStart; if (lt < c.bestLap) c.bestLap = lt; c.lapStart = Date.now();
          return true;
        }
      }
      return false;
    ...
    <Shell title="Racing Game">
      <div className="flex flex-col items-center gap-8 text-center max-w-sm">
        ...
        <div className="flex flex-col gap-3 w-full">
          <button onClick={() => { setGameMode("1p"); setScreen("track-select"); }} className="w-full py-4 bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/50 text-purple-300 font-black rounded-2xl transition-colors">
            🤖 vs Computer AI
            <div className="text-xs font-normal text-muted-foreground mt-1">Choose a circuit · AI difficulty scales with track</div>
          </button>
          <button onClick={() => { setGameMode("2p"); setScreen("track-select"); }} className="w-full py-4 bg-cyan-500/20 hover:bg-cyan-500/30 border border-cyan-500/50 text-cyan-400 font-black rounded-2xl transition-colors">
            👥 2 Players Local
            <div className="text-xs font-normal text-muted-foreground mt-1">P1: Arrow keys · P2: WASD</div>
          </button>
        </div>
      </div>
    </Shell>
        ...
        <div className="flex flex-col gap-2 w-full">
          {TRACKS.map(t => (
            <button key={t.id}
              ...
              className={`w-full py-3 px-5 rounded-2xl border transition-colors font-semibold text-left flex items-center gap-4 ${TRACK_BG[t.stars]} ${TRACK_BORDERS[t.stars]} hover:brightness-125`}
            >
        ...
        {(state === "idle" || state === "done") && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 rounded-xl gap-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => { const t = g.current.track; startRace(gameMode, t); }} className="px-8 py-2 bg-primary text-black font-bold rounded-xl">{state === "done" ? "Race Again" : "Start Race"}</button>
              <button onClick={() => setScreen("track-select")} className="px-6 py-2 bg-secondary text-foreground font-bold rounded-xl">Change Track</button>
              <button onClick={() => setScreen("menu")} className="px-4 py-2 bg-secondary text-foreground font-bold rounded-xl">Menu</button>
            </div>
          </div>
        )}
      </div>
